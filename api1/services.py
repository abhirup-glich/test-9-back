...
class AdminService:
    ...
    def register_student(data):
        ...
        # Retry logic for unique_id collision
        max_retries = 3
        for attempt in range(max_retries):
            unique_id = ''.join(random.choices(string.digits, k=5))
            
            # Check if unique_id exists (optional but good for safety)
            try:
                # We can skip this check if we trust the insert collision error, 
                # but explicit check is safer if unique constraint is missing on DB
                check_uid = supabase.table('students').select('id').eq('unique_id', unique_id).execute()
                if not check_uid.data:
                    break
            except Exception:
                pass # If check fails, try to proceed to insert
        else:
            abort(500, message="Failed to generate unique ID after multiple attempts")
