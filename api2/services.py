...
class AuthService:
    ...
    def register_student(data):
        ...
        if not email or not password or not name or not course or not roll:
            abort(400, message="name, roll, course, email, and password are required")
        ...
        try:
            ...
            # Check for existing roll number (unique_id)
            existing_roll = supabase.table('students').select('id').eq('unique_id', roll).execute()
            ...
            try:
                ...
                return res.data[0] if res.data else new_student
                 ...
                 # Fallback for schema mismatch (password vs password_hash)
                 error_str = str(insert_error)
                 if "password" in error_str and "column" in error_str:
