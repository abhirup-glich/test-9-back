...
class AuthService:
    ...
    def register_student(data):
        ...
        try:
            ...
            # Check for existing roll number (try both schemas)
            try:
                ...
                if "column" in str(e):
                    ...
                    existing_roll = supabase.table('students').select('id').eq('unique_id', roll_number).execute()
                    if existing_roll.data:
                        abort(409, message="Roll number already registered")
                else:
                    raise e
            ...
            try:
                ...
                return res.data[0] if res.data else new_student
                 ...
                 if "column" in error_str and "roll_number" in error_str:
                      ...
                      new_student_fallback = new_student.copy()
                      new_student_fallback['unique_id'] = new_student_fallback.pop('roll_number')
                      
                      try:
                          res = supabase.table('students').insert(new_student_fallback).execute()
                          data = res.data[0] if res.data else new_student_fallback
                          if 'roll_number' not in data and 'unique_id' in data:
                              data['roll_number'] = data['unique_id']
                          return data
                      except Exception as inner_error:
                           # Nested fallback for password
                           inner_error_str = str(inner_error)
                           if "column" in inner_error_str and "password" in inner_error_str:
                                new_student_fallback['password_hash'] = new_student_fallback.pop('password')
                                res = supabase.table('students').insert(new_student_fallback).execute()
                                data = res.data[0] if res.data else new_student_fallback
                                if 'roll_number' not in data and 'unique_id' in data:
                                    data['roll_number'] = data['unique_id']
                 ...
                 elif "password" in error_str and "column" in error_str:
                      print(f"Retrying with password_hash due to error: {error_str}")
                      new_student_fallback = new_student.copy()
                      new_student_fallback['password_hash'] = new_student_fallback.pop('password')
                      res = supabase.table('students').insert(new_student_fallback).execute()
                      return res.data[0] if res.data else new_student_fallback
    ...
    def login_student(email, password):
        ...
        try:
            ...
            user_id = user.get('roll_number') or user.get('unique_id')
